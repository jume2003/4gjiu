    ...
    >
      ...
      <el-table-column :label="$t('table.avatar')" min-width="80px">
        <template slot-scope="{row}">
          <el-popover placement="right" title="" trigger="click">
            ...
            <img slot="reference" :src="row.headimgurl" :alt="row.headimgurl" style="max-height: 40px;max-width: 40px">
          </el-popover>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="300" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <el-button v-if="roles[0]=='admin'" size="mini" type="danger" @click="handleDelete(row,$index)">
            {{ $t('table.delete') }}
          </el-button>
        </template>
      </el-table-column>
...
import { fetchWxUsers, createWxUsers, updateWxUsers, deleteWxUsers } from '@/api/user'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchWxUsers(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createWxUsers({ user: this.temp, token: getToken() }).then((response) => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateWxUsers({ user: tempData, token: getToken() }).then(() => {
    ...
    handleDelete(row, index) {
      deleteWxUsers({ user: this.list[index], token: getToken() }).then(() => {
        this.$notify({
          title: '成功',
          message: '删除成功',
          type: 'success',
